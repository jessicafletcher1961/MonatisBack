...
@Table(
		...
		uniqueConstraints = {
				@UniqueConstraint(
						name = "uk_regle_import_interne_releve",
						columnNames = { "compte_interne_releve_id", "role_compte_contrepartie", "cle_libelle_normalisee" })
		},
		indexes = {
				@Index(
						name = "idx_regle_import_releve_cle_role",
						columnList = "cle_libelle_normalisee, role_compte_contrepartie"),
				@Index(
...
public class RegleImportReleve {
	...
	@JoinColumn(name = "compte_interne_releve_id", foreignKey = @ForeignKey(ConstraintMode.NO_CONSTRAINT))
	@NotFound(action = NotFoundAction.IGNORE)
	private CompteInterne compteInterneReleve;

	@ManyToOne(fetch = FetchType.EAGER)
	@JoinColumn(name = "compte_contrepartie_id", foreignKey = @ForeignKey(ConstraintMode.NO_CONSTRAINT))
	@NotFound(action = NotFoundAction.IGNORE)
	private Compte compteContrepartie;

	@Enumerated(EnumType.STRING)
	@Column(length = 20, nullable = false)
	private RoleCompteContrepartie roleCompteContrepartie;
	...
	public RoleCompteContrepartie getRoleCompteContrepartie() {
		return roleCompteContrepartie;
	}

	public void setRoleCompteContrepartie(RoleCompteContrepartie roleCompteExterne) {
		this.roleCompteContrepartie = roleCompteExterne;
	}
	...
	public CompteInterne getCompteInterneReleve() {
		return compteInterneReleve;
	}

	public void setCompteInterneReleve(CompteInterne compteInterneContexte) {
		this.compteInterneReleve = compteInterneContexte;
	}

	public Compte getCompteContrepartie() {
		return compteContrepartie;
	}

	public void setCompteContrepartie(Compte compteExterne) {
		this.compteContrepartie = compteExterne;
	}
	...
	@Override
	public int hashCode() {
		return Objects.hash(id, cleLibelleNormalisee, roleCompteContrepartie, compteInterneReleve);
	}
	...
	public boolean equals(Object obj) {
		...
		return Objects.equals(id, other.id)
				...
				&& roleCompteContrepartie == other.roleCompteContrepartie
				&& Objects.equals(compteInterneReleve, other.compteInterneReleve);
	}
}
